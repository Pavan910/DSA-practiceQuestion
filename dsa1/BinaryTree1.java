...
public class BinaryTree1 {
    ...
    private void populate(Scanner scanner, Node node) {
        ...
        if(right){
            System.out.println("enter the value right node: "+node.value);
            int value=scanner.nextInt();
    ...
    public  void prettydis(){
        prettydis(root,0);
    }
    private void prettydis(Node node, int level){
        if(node==null){
            return;
        }
        prettydis(node.right, level+1);
        if(level!=0){
